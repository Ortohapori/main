...
const AddPlantacoes: React.FC = () => {
  const [form] = Form.useForm();
  const [plantacoes, setPlantacoes] = useState<Plantacao[]>([]);
  const [hortalicasSelecionadas, setHortalicasSelecionadas] = useState<
    string[]
  >([]);
  const [messageApi, contextHolder] = message.useMessage();

  const onFinish = (values: any) => {
    if (hortalicasSelecionadas.length === 0) {
      messageApi.open({
        type: "error",
        content: "Selecione pelo menos uma hortaliça",
      });
      return;
    }
    setPlantacoes([
      ...plantacoes,
      {
        ...
        hortalicas: hortalicasSelecionadas,
        ...values,
      },
    ]);
    setHortalicasSelecionadas([]);
    form.resetFields();
  };

  const hortaliçasDisponiveis = [
  ...
  return (
    <div>
      {contextHolder}
      <Title level={4}>Adicionar Plantações</Title>
      ...
      <Form form={form} onFinish={onFinish} layout="vertical">
        <Form.Item
          ...
          rules={[
            { required: true, message: "Insira um nome para a plantação" },
          ]}
        >
          <Input placeholder="Ex: Horta de Verão" />
        </Form.Item>
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Selecione pelo menos uma hortaliça",
            },
          ]}
        >
          <Select
            ...
            allowClear
            style={{ width: "100%" }}
            placeholder="Selecione as hortaliças"
            onChange={(values) => setHortalicasSelecionadas(values)}
            value={hortalicasSelecionadas}
          >
            {hortaliçasDisponiveis.map((hortalica) => (
              <Select.Option key={hortalica.value} value={hortalica.value}>
                {hortalica.label}
              </Select.Option>
            ))}
          </Select>
      ...
      {/* Exibir as plantações adicionadas */}
      <Space direction="vertical" style={{ width: "100%" }}>
        {plantacoes.length > 0 && <Title level={4}>Minhas Plantações</Title>}
        {plantacoes.map((plantacao, index) => (
          <Card key={index} title={plantacao.nome}>
            <ul>
              {plantacao.hortalicas.map(
                (hortalica: React.Key | null | undefined) => (
                  <li key={hortalica}>
                    {
                      hortaliçasDisponiveis.find((h) => h.value === hortalica)
                        ?.label
                    }
                    {/* <Button
                      type="link"
